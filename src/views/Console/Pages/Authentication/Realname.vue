<template>
    <div class="realname">
        <p class="console-PageTitle">实名认证<span>（实名认证一经通过，后续付款方的银行开户名必须与聚合实名信息一致，否则不给予入账！）</span></p>
        <div v-if="isrz==1">
            <div class="truerz">
                <p v-if="zhtype==1"><span class="rztitle">认证类型：</span>个人认证</p>
                <p v-if="zhtype==2"><span class="rztitle">认证类型：</span>企业认证</p>
                <p><span class="rztitle">姓名：</span>{{airforce.login.data.user.name}}</p>
                <p v-if="zhtype==1"><span class="rztitle">身份证号码：</span>{{airforce.login.data.user.id_card}}</p>
                <p v-if="zhtype==1"><span class="rztitle">身份证正面：</span><img :src="airforce.login.data.user.card_face" alt=""></p>
                <p v-if="zhtype==1"><span class="rztitle">身份证背面：</span><img :src="airforce.login.data.user.card_back" alt=""></p>
                <p v-if="zhtype==2"><span class="rztitle">公司地址：</span>{{airforce.login.data.user.province}} {{airforce.login.data.user.city}} {{airforce.login.data.user.district}} {{airforce.login.data.user.address}}</p>
                <p v-if="zhtype==2"><span class="rztitle">社会统一编码：</span>{{airforce.login.data.user.world_code}}</p>
                <p v-if="zhtype==2"><span class="rztitle">营业执照：</span><img :src="airforce.login.data.user.business_img" alt=""></p>
                <p v-if="zhtype==2"><span class="rztitle">联系人姓名：</span>{{airforce.login.data.user.contact_name}}</p>
                <p v-if="zhtype==2"><span class="rztitle">联系人电话：</span>{{airforce.login.data.user.contact_phone}}</p>
            </div>



            <!-- <p class="notes">企业名称变更：提供<span class="sign">工商局出示的公司名称变更核准通知书</span>或者<span class="sign">公司准予变更登记通知书</span>（如变更登记通知书涉及到除名称变更外的其他敏感信息，可马赛克相关信息，仅保留名称变更信息）、<span class="sign">名称变更后的营业执照彩色扫描件或照片。注：自2018-08-07起，只支持企业名称变更</span></p> -->
        </div>
        <div v-if="isrz==2">
            <div class="truerz">
                <p class="loading">等待认证中.....</p>
            </div>
        </div>
        <div v-if="isrz==3">
            <div class="chosetype">
                <span>认证类型：</span>
                <label>
                    <input type="radio" v-model="rztype" value="1">
                    <span>个人认证</span>
                </label>
                <label>
                    <input type="radio" v-model="rztype" value="2">
                    <span>企业认证</span>
                </label>
            </div>
            <div class="grrz" v-if="rztype=='1'">
                <!-- <p>基本信息填写</p> -->
                <div class="grrzcontent">
                    <p class="qytitle">基本信息填写</p>
                    <div class="inputpart">
                        <span class="inputtitle">* 姓名：</span>
                        <input type="text" v-model="grname" placeholder="该名称用于发票抬头，请谨慎填写">
                    </div>

                    <div class="inputpart">
                        <span class="inputtitle">* 身份证号码：</span>
                        <input type="text" v-model="grid" placeholder="请填写身份证号码">
                    </div>

                    <div class="filepart">
                        <span class="inputtitle">* 身份证正面：</span>
                        <div class="mainfile">
                            <div class="filebox" v-if="idcardz.show">
                                <span><span class="iconfont">&#xe6a0;</span>选择文件</span>
                                <span>未选择任何文件</span>
                                <input type="file" @change="uploadidz" ref="idz" accept=".png,.jpg,.jpeg">
                            </div>
                            <p  v-if="idcardz.show">图片大小不要超过2M，支持PNG，JPG格式</p>
                            <img class="img" :src="idcardz.url" v-if="!idcardz.show" alt="">
                        </div>
                    </div>

                    <div class="filepart">
                        <span class="inputtitle">* 身份证反面：</span>
                        <div class="mainfile">
                            <div class="filebox" v-if="idcardf.show">
                                <span><span class="iconfont">&#xe6a0;</span>选择文件</span>
                                <span>未选择任何文件</span>
                                <input type="file" @change="uploadidf" ref="idf" accept=".png,.jpg,.jpeg">
                            </div>
                            <p v-if="idcardf.show">图片大小不要超过2M，支持PNG，JPG格式</p>
                            <img class="img" :src="idcardf.url" v-if="!idcardz.show" alt="">
                        </div>
                    </div>

                    <button @click.prevent="grsubmit" class="submitbtn">提交认证</button>

                    <div class="explainpart">
                        <span class="inputtitle">示例图片：</span>
                        <img :src="require('@/assets/img/renzhen/idcard.png')" alt="">
                    </div>
                    <div class="explainpart">
                        <span class="inputtitle">认证说明：</span>
                        <div class="explainbox">
                            <p>用户在使用本平台完成应用或网站开发并上线，可提交认证审核。开发期间（2个月），未审核的应用并不影响接口的调用；</p>
                            <p>为了使本平台更了解开发者的实际使用场景，为后续提供更好更优质的服务，请各开发者完善开发的应用的基本信息；</p>
            ...
            <div class="qyrz" v-if="rztype=='2'">
                <div class="jbxx qypart">
                    <p class="qytitle">企业基本信息填写</p>
                    <div class="inputpart">
                        <span class="inputtitle">* 公司名称：</span>
                        <input type="text" v-model="gsxx.name">
                    </div>
                    <div class="inputpart">
                        <span class="inputtitle">* 公司地址：</span>
                        <div class="address">
                            <select @change="showCity" v-model="prov">
                                <option v-for="(item,index) in adrprov" :key="index" :value="index">{{item}}</option>
                            </select>
                            <select @change="showCountry" v-model="city">
                                <option v-for="(item,index) in adrcity" :key="index" :value="index">{{item}}</option>
                            </select>
                            <select @change="selecCountry" v-model="country">
                                <option v-for="(item,index) in adrcountry" :key="index" :value="index">{{item}}</option>
                            </select>
                        </div>
                    </div>
                    <div class="inputpart">
                        <span class="inputtitle">* 详细地址：</span>
                        <input type="text" v-model="gsxx.address">
                    </div>
                </div>

                <div class="qyxx qypart">
                    <p class="qytitle">企业信息</p>
                    <div class="inputpart">
                        <span class="inputtitle">* 社会统一编码：</span>
                        <input type="text" v-model="gsxx.code">
                    </div>
                    <!-- <div class="inputpart">
                        <span class="inputtitle">税务登记证编号：</span>
                        <input type="text" >
                    </div>
                    <div class="inputpart">
                        <span class="inputtitle">组织机构编号：</span>
                        <input type="text" >
                    </div> -->
                    <div class="filepart">
                        <span class="inputtitle">* 营业执照副本：</span>
                        <div class="mainfile">
                            <div class="filebox" v-if="yyzz.show">
                                <span><span class="iconfont">&#xe6a0;</span>选择文件</span>
                                <span>未选择任何文件</span>
                                <input type="file" @change="uploadyyzz" ref="yyzz" accept=".png,.jpg,.jpeg">
                            </div>
                            <p v-if="yyzz.show">图片大小不要超过2M，支持PNG，JPG格式</p>
                            <img class="img" :src="yyzz.url" v-if="!yyzz.show" alt="">
                        </div>
                    </div>
                    <div class="inputpart">
                        <span class="inputtitle">* 法人姓名：</span>
                        <input type="text" v-model="gsxx.frname">
                    </div>
                    <div class="inputpart">
                        <span class="inputtitle">* 法人电话：</span>
                        <input type="text" v-model="gsxx.frtel">
                    </div>
                </div>

                <div class="lxrxx qypart">
                    <p class="qytitle">联系人信息</p>
                    <div class="inputpart">
                        <span class="inputtitle">* 联系人姓名：</span>
                        <input type="text" v-model="gsxx.lxname">
                    </div>
                    <div class="inputpart">
                        <span class="inputtitle">* 联系人电话：</span>
                        <input type="text" v-model="gsxx.lxtel">
                    </div>
                    
                </div>
                
                <button @click.prevent="qysubmit" class="submitbtn">提交认证</button>
            </div>
        </div>
    </div>
...
export default {
    ...
    computed:{
        isrz(){
            if(this.airforce.login.data.user.is_name_true==3){//已认证
                return 1;
            }else if(this.airforce.login.data.user.is_name_true==2){//认证中
                return 2;
            }else{//未认证
                return 3;
            }
        ...
        zhtype(){
            if(this.airforce.login.data.user.type==1){//个人
                return 1;
            }else{//企业
                return 2;
            }
    ...
    methods:{
        showCity(){//省改变的方法
            if(this.prov==0){
                ...
                this.adrcity=["=请选择城市="];
                this.city=0;
                for(let i=0;i<this.addresslist[this.prov-1].city.length;i++){
                    this.adrcity.push(this.addresslist[this.prov-1].city[i].name)
                }
                this.provtext=this.addresslist[this.prov-1].name;
                // this.citytext=this.addresslist[this.prov-1].city[0].name;
                // this.countrytext="";
            }
            
            console.log(this.provtext,this.citytext,this.countrytext)
        },
        showCountry(){//市改变的方法
            //console.log(this.addresslist[this.prov-1].city[this.city-1].districtAndCounty)
            if(this.city==0){
                this.adrcountry=["=请选择县区="];
                this.citytext="";
                this.countrytext="";
                ...
                this.adrcountry=['=请选择县区='];
                let newarr=this.addresslist[this.prov-1].city[this.city-1].districtAndCounty;
                for(let i=0;i<newarr.length;i++){
                    this.adrcountry.push(newarr[i]);
                }
                this.citytext=this.addresslist[this.prov-1].city[this.city-1].name;
                //this.countrytext=this.addresslist[this.prov-1].city[this.city-1].districtAndCounty[0];
                console.log(this.provtext,this.citytext,this.countrytext)
            }
        },
        selecCountry(){//区改变的方法
            if(this.country==0){
                ...
                this.countrytext=this.addresslist[this.prov-1].city[this.city-1].districtAndCounty[this.country-1];
            }
            
            console.log(this.provtext,this.citytext,this.countrytext)
        },
        grsubmit(){//个人认证的提交按钮
            let sfzz=/(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/;
            if(this.grname==" "){
                this.$vux.toast.text("姓名项不能为空");
                return;
            }
            if(this.grid==" "){
                this.$vux.toast.text("身份证项不能为空");
                return;
            }
            if(!sfzz.test(this.grid)){
                this.$vux.toast.text("请输入正确的身份证号");
                return;
            }
            if(this.idcardz.url==""){
            ...
            this.action({
                ...
                url:"/user/User/authentication",
                method:"post",
                headers: {'XX-token': this.airforce.login.data.token,},
                isFormData:true,
                data:{
                    type:"1",
                    id:this.airforce.login.data.user.id,
                    name:this.grname,
                    id_card:this.grid,
                    card_face:this.idcardz.file,
                    card_back:this.idcardf.file,
                }
            }).then(res=>{
                if(res.code==200){
                    //写入本地实名信息
                    let smxx=JSON.parse(JSON.stringify(this.airforce.login.data));
                    smxx.user.name=res.data.name;
                    smxx.user.id_card=res.data.id_card;
                    smxx.user.card_face=res.data.card_face;
                    smxx.user.card_back=res.data.card_back;
                    smxx.user.is_name_true='2';
        ...
        qysubmit(){//企业认证的提交按钮
            ...
            if(this.provtext==""){
                this.$vux.toast.text("请选择省级");
                return;
            }
            if(this.citytext==""){
                this.$vux.toast.text("请选择市级");
                return;
            }
            if(this.gsxx.address==""){
                this.$vux.toast.text("请输入公司详细地址");
                return;
            }
            if(this.gsxx.code==""){
                this.$vux.toast.text("请输入企业社会统一编码");
                return;
            }
            if(this.yyzz.url==""){
                this.$vux.toast.text("请上传企业营业执照副本");
                return;
            }
            if(this.gsxx.frname==""){
                this.$vux.toast.text("请输入法人姓名");
                return;
            }
            if(this.gsxx.frtel==""){
                this.$vux.toast.text("请输入法人电话");
                return;
            }
            if(this.gsxx.lxname==""){
                this.$vux.toast.text("请输入联系人姓名");
                return;
            }
            if(this.gsxx.lxtel==""){
                this.$vux.toast.text("请输入联系人电话");
                return;
            }
            this.action({
                moduleName:'authentication',
                ...
                data:{
                    ...
                    city:this.citytext,
                    district:this.countrytext,
                    address:this.gsxx.address,
                    world_code:this.gsxx.code,
                    business_img:this.yyzz.file,
                    contact_name:this.gsxx.lxname,
                    contact_phone:this.gsxx.lxtel,
                    legal_person:this.gsxx.frname,
                    legal_person_phone:this.gsxx.frtel,
                }
            }).then(res=>{
                if(res.code==200){
                    let smxx=JSON.parse(JSON.stringify(this.airforce.login.data));
                    smxx.user.name=res.data.name;
                    smxx.user.province=res.data.province;
                    smxx.user.city=res.data.city;
                    smxx.user.district=res.data.district;
                    smxx.user.address=res.data.address;
                    smxx.user.world_code=res.data.world_code;
                    smxx.user.business_img=res.data.card_face;
                    smxx.user.contact_name=res.data.contact_name;
                    smxx.user.contact_phone=res.data.contact_phone;
                    smxx.user.is_name_true='2';
                    this.$utils.updataUserInfo(this,{
                        ...smxx.user
                    });
        ...
        uploadidz(){//上传身份证正面
            var fileObj = this.$refs.idz.files[0]; //获取到files的信息
            this.idcardz.file=fileObj;
            var reader = new FileReader();
            reader.readAsDataURL(fileObj);
            var that = this;
            reader.onload = function(x){
                that.idcardz.show=false;
                that.idcardz.url=this.result;
                // console.log(this.result)
            }
        },
        uploadidf(){//上传身份证反面
            var fileObj = this.$refs.idf.files[0]; //获取到files的信息
            this.idcardf.file=fileObj;
            var reader = new FileReader();
            reader.readAsDataURL(fileObj);
            var that = this;
            reader.onload = function(x){
                that.idcardf.show=false;
                that.idcardf.url=this.result;
            }
        },
        uploadyyzz(){
            var fileObj = this.$refs.yyzz.files[0]; //获取到files的信息
            this.yyzz.file=fileObj;
            var reader = new FileReader();
            reader.readAsDataURL(fileObj);
            var that = this;
            reader.onload = function(x){
                that.yyzz.show=false;
                that.yyzz.url=this.result;
            }
        }
    },
    mounted(){
        ...
        for(let i=0;i<this.addresslist.length;i++){
            this.adrprov.push(this.addresslist[i].name)
        }
        this.action({
            moduleName:"isRealname",
            url:"usersmsapilist",
            headers:{"XX-token":this.airforce.login.data.token},
            method:"post",
            data:{
            ...
            if(res.code==200){
                this.$utils.updataUserInfo(this,{
                    is_name_true:res.data[0].is_name_true,
                    type:res.data[0].user_check_type,
                    name:res.data[0].name,
                    id_card:res.data[0].id_card,
                    card_face:res.data[0].card_face,
                    card_back:res.data[0].card_back,
                    province:res.data[0].province,
                    city:res.data[0].city,
                    district:res.data[0].district,
                    address:res.data[0].address,
                    world_code:res.data[0].world_code,
                    business_img:res.data[0].business_img,
                    contact_name:res.data[0].contact_name,
                    contact_phone:res.data[0].contact_phone,
                });
            }else{
                this.$vux.toast.text(res.msg)
            }
