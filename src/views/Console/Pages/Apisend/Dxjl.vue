...
export default {
    ...
    methods:{
        ...
        screenfn(){//点击搜索按钮的方法
            ...
            if(this.tabval=="1"){//接受记录的数据
                this.getjstabledata();
            }else if(this.tabval=="2"){//回复记录的数据
            ...
            this.action({
                ...
                if(res.code==200){
                    ...
                    for(let i=0;i<res.data.length;i++){
                        let telist=res.data[i].phone;
                        for(let o=0;o<telist.length;o++){
                            let newobj={};
                            newobj.pcnum=res.data[i].sms_order_sn;
                            newobj.content=res.data[i].sms_content;
                            newobj.fsscore=res.data[i].send_time;
                            newobj.tel=telist[o];
                            newobj.belong=res.data[i].province[o];
                            newobj.operator=res.data[i].sp[o];
                            newarr.push(newobj);
                        }
                    }
                    this.rowsjs=newarr;
